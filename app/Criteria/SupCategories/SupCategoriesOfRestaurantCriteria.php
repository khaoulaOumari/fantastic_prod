 ...
 */
class SupCategoriesOfRestaurantCriteria implements CriteriaInterface
{
    /**
     * @var array
     */
    ...
    /**
     * SupCategoriesOfRestaurantCriteria constructor.
     */
    ...
    public function apply($model, RepositoryInterface $repository)
    {
        if (!$this->request->has('restaurant_id')) {
            ...
            return $model->join('sup_categories','categories.supcategory_id','sup_categories.id')
                ->join('foods', 'foods.category_id', '=', 'categories.id')
                ->where('foods.restaurant_id', $id)
                ->select('sup_categories.*')
                ->groupBy('sup_categories.id');
        }
    }
}
